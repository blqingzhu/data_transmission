...
import redis
# 连接数据库
r = redis.Redis(host='r-m5el1rv4i4t6k47cghpd.redis.rds.aliyuncs.com', port=6379,db=0)
# 使用连接池连接数据库。这样就可以实现多个Redis实例共享一个连接池
pool = redis.ConnectionPool(decode_response=True,host='127.0.0.1', port=6379)
r = redis.Redis(connection_pool=pool,host='r-m5el1rv4i4t6k47cghpd.redis.rds.aliyuncs.com', port=6379,password='FvWO2WqoKbyfS3ji',db=0)
